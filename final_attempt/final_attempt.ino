...
void loop() {
  ...
  if (digitalRead(BUTTON_PIN) == LOW) {
    // Iterate through melody and note durations arrays
    for (int i = 0; i < sizeof(melody) / sizeof(melody[0]); i++) {
      // Play the note
      tone(BUZZER_PIN, melody[i], 500 / noteDurations[i]);
      // Delay between notes
      delay(500 / noteDurations[i] * 1.70);
    }
    // Turn off the buzzer
    noTone(BUZZER_PIN);
    // Delay before replaying the melody
    delay(1000);
  }
}
